...
let createBar = function(data) {
    ...
    const update_scale = () => {
        ...
        bar_svg.select('#x_scale').transition().duration(500).call(d3.axisBottom(x_scale)).attr('font-size', 12);

        y_scale.domain([0, d3.max(Object.values(grouped))]);
        bar_svg.select('#y_scale').transition().duration(500).call(d3.axisLeft(y_scale)).attr('font-size', 12);
    };
